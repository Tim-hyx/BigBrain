...
const postTry = async (path, status, payload, token) =>
  sendTry('post', path, status, payload, token);
const getTry = async (path, status, payload, token) =>
  sendTry('get', path, status, payload, token);
const deleteTry = async (path, status, payload, token) =>
  sendTry('delete', path, status, payload, token);
const putTry = async (path, status, payload, token) =>
...
describe('Test the root path', () => {
  ...
  ***************************************************************/
  ...
  test('Logout a valid session', async () => {
    const bodyLogout = await postTry(
      '/admin/auth/logout',
      200,
      {},
      await validToken()
  ...
  ***************************************************************/
  ...
  test('Try and delete a quiz with invalid quizid', async () => {
    const body = await deleteTry(
      `/admin/quiz/${99999999999999}`,
      400,
      {},
      await validToken()
    );
  ...
  ***************************************************************/
  ...
  test('Ensure that the session appears in old sessions', async () => {
    const quizid = await singleQuizId();
    const { oldSessions, } = await getTry(
      `/admin/quiz/${quizid}`,
      200,
      {},
      await validToken()
  ...
  test('Players should be able to get their results', async () => {
    for (const playerId of PLAYER_IDS) {
      ...
      expect(body).toMatchObject(
        QUESTIONS.map((q) => ({ answerIds: [0, 1, 2,], correct: false, }))
      );
    }
  });
  ...
  test('Admins can get the session results', async () => {
    ...
    const body = await getTry(
      `/admin/session/${oldSessions[0]}/results`,
      200,
      {},
      await validToken()
    );
